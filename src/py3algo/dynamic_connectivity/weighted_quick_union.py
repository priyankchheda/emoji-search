...
QuickFind and QuickUnion algorithms are easy to implement, but simply can't
support huge dynamic connectivity problems.
...
class WeightedQuickUnion:
    """ Same as quick-union, but maintain extra array sz[i] to count number
    ...
    def union(self, elem1, elem2):
        ...
        if self.sz[root_elem1] < self.sz[root_elem2]:
            self.data[root_elem1] = root_elem2
            self.sz[root_elem2] += self.sz[root_elem1]
        else:
            self.data[root_elem2] = root_elem1
            self.sz[root_elem1] += self.sz[root_elem2]
